...
const resolveProductPromoPlan = (productPromo: any) => {
  ...
  return {
    workflowMode,
    renderMode: renderMode as 'image_slide' | 'ai_video',
    targetCuts: 5,
    hookVideoCount: renderMode === 'ai_video' ? 5 : 0,
...
{"hookTitle":"...","tone":"...","audience":"...","scriptHint":"...","visualGuide":"...","productAnchor":"...","detectedTexts":"...","productName":"...","modelNumber":"...","usagePurpose":"...","usageScenarios":"...","ctaLine":"...","pinnedCommentCta":"..."}`,
      ...
      if (productMatch.level === 'low' && !forceContinue) {
        showNotice(`상품 일치도 ${productMatch.score}점: 데이터가 부족해도 자동 제작을 계속 진행합니다.`, 'info', 1500);
        appendAutoLog(`상품 일치도 낮음(${productMatch.score}) - 자동 제작 계속 진행`);
      }

      if (productMatch.level === 'medium' && !forceContinue) {
        showNotice(`상품 일치도 ${productMatch.score}점: 검수 권장 상태로 자동 제작을 진행합니다.`, 'info', 1200);
      }
...
8. 불필요한 설명 없이 1~2문장의 영어 프롬프트만 출력하세요.`;
  ...
  const runPublishAttempt = async (jobId: string, attemptIndex: number) => {
    const latestUi = latestUiRef.current;
    const target = latestUi.publishing.jobs.find((job: any) => job.id === jobId);
    if (!target || target.status === 'published') return;

    const nextAttemptCount = attemptIndex + 1;

    // 메타데이터·영상 파일 검증
    if (target.platform !== 'youtube' || !target.videoUrl || !target.title?.trim() || !target.description?.trim()) {
      const now = new Date().toISOString();
      setUi(prev => ({
        ...prev,
        publishing: {
          ...prev.publishing,
          jobs: prev.publishing.jobs.map((job: any) =>
            job.id !== jobId ? job : {
              ...job,
              status: 'failed' as PublishJobStatus,
              attemptCount: nextAttemptCount,
              lastError: '메타데이터 또는 영상 파일 누락으로 발행 실패',
              notifiedAt: now,
            }
          ),
        },
    ...
    // setTimeout 콜백에서 최신 auth를 얻기 위해 localStorage에서 직접 읽음
    let auth: YouTubeAuthSession | null = null;
    try {
      const raw = localStorage.getItem(YT_AUTH_SESSION_LS_KEY);
      auth = raw ? (JSON.parse(raw) as YouTubeAuthSession) : null;
    } catch { /* ignore */ }
        ...
        publishing: {
          ...
          jobs: prev.publishing.jobs.map((job: any) =>
            job.id !== jobId ? job : {
              ...
              status: 'failed' as PublishJobStatus,
              attemptCount: nextAttemptCount,
              lastError: 'YouTube 인증이 만료되었습니다. 계정을 다시 연결해 주세요.',
              notifiedAt: now,
            }
          ),
        },
    ...
    setUi(prev => ({
      ...
      publishing: {
        ...prev.publishing,
        jobs: prev.publishing.jobs.map((job: any) =>
          job.id !== jobId ? job : { ...job, status: 'publishing' as PublishJobStatus, lastError: '업로드 준비 중...' }
        ),
      },
    }));

    try {
      // Step 1: 영상 Blob 가져오기 (object URL 또는 blob URL)
      const videoRes = await fetch(target.videoUrl);
      if (!videoRes.ok) throw new Error('영상 파일을 불러올 수 없습니다.');
      const videoBlob = await videoRes.blob();
      const mimeType = videoBlob.type || 'video/webm';

      // Step 2: YouTube Resumable Upload 세션 초기화
      const metadata = {
        snippet: {
          title: target.title,
        ...
        status: {
          privacyStatus: target.visibility || 'private',
      ...
      if (!initRes.ok) {
        ...
        throw new Error(`업로드 세션 생성 실패 (${initRes.status}): ${errText.slice(0, 200)}`);
      }

      const uploadUrl = initRes.headers.get('Location');
      if (!uploadUrl) throw new Error('업로드 URL을 받지 못했습니다.');

      // Step 3: XHR로 영상 업로드 (진행률 추적)
          ...
          if (e.lengthComputable) {
            ...
            setUi(prev => ({
              ...
              publishing: {
                ...prev.publishing,
                jobs: prev.publishing.jobs.map((job: any) =>
                  job.id !== jobId ? job : { ...job, lastError: `업로드 중... ${pct}%` }
                ),
              },
            }));
          }
      ...
      // 업로드 성공
      const now = new Date().toISOString();
      const publishedUrl = `https://youtube.com/watch?v=${videoId}`;
      setUi(prev => ({
        ...prev,
        publishing: {
          ...prev.publishing,
      ...
      clearPublishTimers(jobId);
      setTimeout(() => sendPublishEmailNotification({ ...target, status: 'published', publishedUrl, notifiedAt: now }), 0);

    } catch (err: any) {
      const errorMsg = String(err?.message || err || '알 수 없는 오류');
      const now = new Date().toISOString();
      const shouldRetry = target.autoRetry && nextAttemptCount < target.maxAttempts;
      ...
      setUi(prev => ({
        ...
        publishing: {
          ...
          jobs: prev.publishing.jobs.map((job: any) =>
            job.id !== jobId ? job : {
              ...job,
              status: (shouldRetry ? 'scheduled' : 'failed') as PublishJobStatus,
              attemptCount: nextAttemptCount,
              lastError: errorMsg,
              notifiedAt: shouldRetry ? '' : now,
            }
          ),
        },
      }));
      ...
      if (shouldRetry) {
        ...
        clearPublishTimers(jobId);
        setTimeout(() => sendPublishEmailNotification({ ...target, status: 'failed', lastError: errorMsg, notifiedAt: now }), 0);
      }
    }
  };
  ...
  const handleGenerateFinalVideo = async () => {
    ...
    const desiredHookCount = isProductPromoContext
      ? (productPlan.renderMode === 'ai_video'
        ? Math.min(Number(productPlan.hookVideoCount || 0), sortedVideoJobs.length)
...
${JSON.stringify(cutPayload)}`,
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-6 py-10 space-y-8 relative z-10">
      ...
      <section className="max-w-5xl mx-auto bg-white/5 border border-white/10 rounded-3xl p-4 md:p-5 backdrop-blur-xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">상품쇼츠는 5컷·20초(컷당 4초)로 고정됩니다. 출력 방식(이미지 슬라이드/AI 비디오)만 선택하면 실행됩니다.</p>
          </div>
          <button
            onClick={() => {
              void runProductPromoOneClick(true);
            }}
          ...
          >
            {ui.productPromo.running ? '상품쇼츠 제작 중...' : '상품쇼츠 실행'}
          </button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
          <div className="rounded-xl border border-white/10 bg-black/20 p-3 space-y-2">
            <p className="text-[10px] font-black text-fuchsia-200 uppercase tracking-widest">실행 모드</p>
            ...
            <p className="text-[10px] text-slate-500">두 모드 모두 상품쇼츠 전용 5컷·20초 동일 규칙으로 실행됩니다.</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-black/20 p-3 space-y-2">
          ...
          <div className="rounded-xl border border-white/10 bg-black/20 p-3 space-y-2">
            ...
            <p className="text-[10px] text-slate-500">
              {productPromoPlan.renderMode === 'ai_video'
                ? 'AI비디오: 이미지 5컷 생성 후 각 컷을 4초 영상으로 생성합니다.'
                : '이미지슬라이드: 입력 자산 기반으로 이미지 5컷을 생성해 4초씩 구성합니다.'}
            </p>
          </div>
          <div className="rounded-xl border border-white/10 bg-black/20 p-3 space-y-2">
            <p className="text-[10px] font-black text-fuchsia-200 uppercase tracking-widest">출력 요약</p>
            <div className="flex items-center justify-between text-[10px] text-slate-300">
              <span>총 컷/길이</span>
              <span className="font-black text-white">5컷 · 20초</span>
            </div>
            <p className="text-[10px] text-slate-500">모든 컷은 4초 고정입니다. 출력 방식에 따라 이미지 또는 영상으로 렌더됩니다.</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-black/20 p-3 space-y-2">
              ...
              <label className="text-[10px] text-slate-300 space-y-1">
                ...
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className={`w-full px-3 py-2 rounded-lg border text-[10px] font-black transition-all ${previewingId === (ui.tts.activeProvider === 'elevenlabs' ? ui.tts.elevenlabsVoice : ui.tts.voice) ? 'bg-amber-400 text-black border-amber-300' : 'bg-white/10 text-white border-white/15 hover:bg-white/20'}`}
                  >
        ...
        <div className="grid grid-cols-1 md:grid-cols-[260px_1fr] gap-4">
          <div className="space-y-3">
            ...
            <div className="rounded-lg border border-white/10 bg-black/30 p-2 space-y-1">
              ...
              <p className="text-[10px] text-slate-400">
                상품쇼츠 공통: 5컷·20초 고정 (컷당 4초)
              </p>
              <p className="text-[10px] text-slate-400">이미지 슬라이드: 입력 자산 기반으로 이미지 5컷을 생성해 4초씩 구성</p>
              <p className="text-[10px] text-slate-400">AI 비디오: 이미지 5컷 생성 후 5컷 모두 4초 영상으로 생성</p>
          ...
          <div className="rounded-2xl border border-white/10 bg-black/20 p-4 space-y-2">
            ...
            {productPromoPlan.renderMode === 'image_slide' && (
              <p className="text-[10px] text-slate-500">이미지 슬라이드 모드는 5컷 이미지를 자동 생성해 구성합니다. 영상 자산이 없어도 실패하지 않습니다.</p>
            )}
      ...
      <div className="fixed right-2 md:right-3 bottom-20 md:bottom-auto md:top-1/2 md:-translate-y-1/2 z-[140]">
