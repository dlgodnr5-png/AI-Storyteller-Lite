...
결과 예시: "신뢰감 있고 차분한 목소리로 지적인 분석가처럼 읽어주세요."`,
  ...
  const runOneClickFromTitle = async (
    ...
    try {
      ...
      alert('자동 진행은 12번(슬라이드 구성)까지 완료되었습니다. 13번에서 편집 확인 후 14번에서 최종 발행을 진행하세요.');
      return true;
    } catch (err: any) {
      console.error(err);
  ...
  const runProductPromoOneClick = async () => {
    ...
    try {
      const ai = new GoogleGenAI({ apiKey: keys.g1 });
      const hasInlineProductImage = /^data:image\//i.test(String(ui.productPromo.imageUrl || ''));
      const mimeMatch = hasInlineProductImage ? ui.productPromo.imageUrl.match(/^data:(.*?);base64,/i) : null;
      const mimeType = mimeMatch?.[1] || 'image/jpeg';
      const imageBase64 = hasInlineProductImage ? (ui.productPromo.imageUrl.split(',')[1] || '') : '';
...
{"hookTitle":"...","tone":"...","audience":"...","scriptHint":"...","visualGuide":"...","productAnchor":"...","detectedTexts":"..."}`,
              ...
              ...(hasInlineProductImage ? [{ inlineData: { mimeType, data: imageBase64 } }] : []),
            ],
          },
        ],
        config: { responseMimeType: 'application/json' },
...
7. 불필요한 설명 없이 1~2문장의 영어 프롬프트만 출력하세요.`;
  ...
  const generateImage = async (cutIndex: number, options?: { force?: boolean }) => {
    ...
    if (latest?.productPromo?.strictProductLock && latest?.productPromo?.imageUrl) {
      ...
      setUi(prev => ({
        ...
        imageJobs: prev.imageJobs.some(j => j.cut === cutIndex)
          ? prev.imageJobs.map(j => j.cut === cutIndex ? { ...j, status: '원본 고정', imageUrl: lockedImage } : j)
          : [...prev.imageJobs, { cut: cutIndex, status: '원본 고정', imageUrl: lockedImage }],
      }));
      return;
    }
    ...
    try {
      ...
      const productDataUrl = String(latest?.productPromo?.imageUrl || '').trim();
      const hasProductReference = productDataUrl.startsWith('data:image/');
      const productRefText = hasProductReference
        ? 'Use the reference product image as strict anchor. Do not change product shape, cap, color, logo, package text, label layout. Only change surrounding environment, camera angle, and background context.'
        : '';
      ...
      const res = await generateContentWithFallback(ai, {
        ...
        contents: {
          parts: [
            { text: `${promptObj.prompt}. Style: ${stylePrompt}. ${productRefText}`.trim() },
      ...
      <section className="max-w-5xl mx-auto bg-white/5 border border-white/10 rounded-3xl p-4 md:p-5 backdrop-blur-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
          ...
          <div className="rounded-xl border border-white/10 bg-black/20 p-3 space-y-2">
            ...
            >
              {ui.productPromo.strictProductLock ? 'ON (제품 그대로 유지)' : 'OFF (배경/제품 생성 허용)'}
            </button>
            <p className="text-[10px] text-slate-500">ON이면 자동 생성 시 제품 이미지를 컷에 그대로 고정 사용합니다.</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-black/20 p-3 space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
          ...
          <div className="rounded-xl border border-white/10 bg-black/20 p-3 space-y-2">
            <label className="text-[10px] font-black text-fuchsia-200 uppercase tracking-widest">상품 대표 이미지 URL</label>
            <input
              type="url"
              value={ui.productPromo.sourceImageUrl || ''}
              onChange={(e) => setUi(prev => ({ ...prev, productPromo: { ...prev.productPromo, sourceImageUrl: e.target.value } }))}
              placeholder="https://.../product-main.jpg"
              className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-xs text-white outline-none"
            />
            <button
              onClick={applyProductSourceImageUrl}
              className="w-full py-2 rounded-lg bg-fuchsia-400 text-black text-xs font-black"
            >
              URL 이미지 적용
            </button>
            <p className="text-[10px] text-slate-500">쿠팡/스마트스토어 원본 이미지를 직접 붙여 넣으면 제품 변형 없이 고정 활용됩니다.</p>
          </div>
        </div>
